...
Namespace MyNamespace
    ...
    Public Class MainClass
                ...
                Try
                    ...
                    For Each id As ObjectId In res.Value.GetObjectIds()
                        ...
                        For i = 1 To EndCell
                            ...
                            ed.WriteMessage("REALLENGTHHHHHHHHHHHHHHHHH" & vbLf)

                            If verticesInsideBoundary.Count > 0 Then
                                ' Initialize variables for tracking min and max values
                                ...
                                ' Iterate over the list of coordinates to find the min and max x and y values
                                ...
                                ' Calculate realLength and realWidth
                                realLength = Math.Round(maxX - minX)
                                realWidth = Math.Round(maxY - minY)

                                ' Example usage: Printing the calculated dimensions
                                ed.WriteMessage($"Real Length: {realLength}{Environment.NewLine}")
                                ed.WriteMessage($"Real Width: {realWidth}{Environment.NewLine}")
                            Else
                                ed.WriteMessage("No vertices inside the boundary." & Environment.NewLine)
                            End If


                            Dim ids = psr.Value.GetObjectIds()
                            ed.WriteMessage($"LayerValues:")
                            For Each str As String In LayerValues
                            ...
                            For Each id1 In ids
                                ...
                                Select Case dbobj.GetType()
                                    ...
                                    Case GetType(DBText)
                                        ...
                                        'End If
                                        ed.WriteMessage($"DBText.LayerValues:")
                                        For Each str As String In LayerValues
                                            ed.WriteMessage($"{str}{vbLf}")
                                        Next
                                        ed.WriteMessage($"DBText.StringValues:")
                                        For Each str As String In StringValues
                                            ed.WriteMessage($"{str}{vbLf}")
                                        Next
                                        ed.WriteMessage($"DBText.ObjString:{ObjString}{vbLf}")
                                        ed.WriteMessage($"DBText.layername:{LayerName}{vbLf}")
                                    ...
                                    Case GetType(RotatedDimension)
                                        ...
                                        ed.WriteMessage($"RotatedDimension.LayerValues:")
                                        For Each str As String In LayerValues
                                            ed.WriteMessage($"{str}{vbLf}")
                                        Next
                                        ed.WriteMessage($"RotatedDimension.StringValues:")
                                        For Each str As String In StringValues
                                            ed.WriteMessage($"{str}{vbLf}")
                                        Next
                                        ed.WriteMessage($"RotatedDimnesion.ObjValue:{ObjValue}{vbLf}")
                                        ed.WriteMessage($"RotatedDimnesion.layername:{layername}{vbLf}")
        ...
        End Sub
        <CommandMethod("RowAutoLayout")>
        Public Shared Sub MatrixToLayout2()
            Const VPCenterX As Double = 210.6893
            Const VPCenterY As Double = 172.6984
            Const VPCenterZ As Double = 0
            Dim VPHeight As Double = 234
